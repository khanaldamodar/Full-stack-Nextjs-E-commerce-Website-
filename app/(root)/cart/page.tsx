...
//   const { cart, isLoaded, removeFromCart, updateQuantity, clearCart, subtotal, tax, total } = useCart()
 const { cart, removeFromCart, updateQuantity, clearCart, subtotal, tax, total, isLoaded } = useCartContext()
        ...
        {cart.length === 0 ? (
          <div className="flex h-96 items-center justify-center rounded-lg border border-border bg-card">
            <div className="text-center">
              ...
              <p className="mt-2 text-sm text-muted-foreground">Start shopping to add items to your cart</p>
              <Link href="/products">
          ...
          <div className="grid gap-8 lg:grid-cols-3">
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1">
                        <Link href={`/products/${item.id}`}>
                          <h3 className="font-semibold text-foreground hover:text-primary">{item.name}</h3>
                        </Link>
                        <p className="text-sm text-muted-foreground">{item.category}</p>
                        <p className="mt-2 text-lg font-bold text-foreground">Rs. {item.price.toFixed(2)}</p>
                      </div>
            ...
            <div className="lg:col-span-1">
              <Card className="sticky top-4 p-6">
                ...
                <Button variant="outline" className="mt-3 w-full bg-red-500" onClick={handleClearcart}>
                  Clear Cart
                </Button>
